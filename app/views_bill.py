...
def addcard(request,pk=None):
    if request.POST:
        ...
        if _bills:
            return render(request,'bill/addcard.html',{'bills':_bills})
        else:
...
def addonbill(request,pk=None):
    if request.POST:
        ...
        _bills=Bill.objects.all().filter(client=_user)
        if _bills:
            return render(request,'bill/addonbill.html',{'bills':_bills})
        else:
            return addbill(request,pk)
...
def addbill(request,pk=None):
    ...
    if request.POST:
        ...
        return render(request,'bill/addbill.html',{'bill_user':_user})
